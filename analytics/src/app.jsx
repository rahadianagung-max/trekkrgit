...
/* Trekkr Sabermetrics v5 — Overview with two entry points
 * (Explore by venue / Explore by players). Venue mode = whole-roster
 * analysis; Players mode = global search. Player detail restores full
 * partner analysis (best partners + lifts / holds you back). Live-themed
 * to trekkr.online. Real data embedded below. */
...
function Icon({ name }) {
  ...
  if (name === "venue") return <svg width="20" height="20" viewBox="0 0 24 24"><path d="M4 20V9l8-5 8 5v11" {...p} /><path d="M9 20v-6h6v6" {...p} /></svg>;
...
function ExplainSheet({ open, onClose }) {
  ...
  return <div onClick={onClose} style={{ position: "fixed", inset: 0, background: "#0D0D0D55", zIndex: 50, display: "flex", alignItems: "flex-end", justifyContent: "center", animation: "fadeIn .15s ease" }}>
    <div onClick={(e) => e.stopPropagation()} style={{ width: "100%", maxWidth: 440, background: T.surf, borderTop: `3px solid ${T.orange}`, borderRadius: "18px 18px 0 0", padding: "20px 20px 28px", animation: "sheetUp .22s cubic-bezier(.2,.8,.2,1)", maxHeight: "82vh", overflowY: "auto", boxShadow: "0 -20px 60px rgba(13,13,13,.18)" }}>
...
function Intro({ go, onOpen }) {
  ...
  return <div style={{ padding: "8px 18px 40px", animation: "fadeIn .25s ease" }}>
    <div style={{ padding: "26px 0 20px" }}>
      <div style={{ ...eyebrow, color: T.orange, marginBottom: 14 }}>New · Advanced analytics</div>
      <h1 style={{ ...head, fontSize: 52, margin: 0, color: T.text }}>See what<br />win rate <span style={gradText}>hides</span></h1>
      <p style={{ ...body, fontSize: 15, color: T.muted, lineHeight: 1.6, marginTop: 16, maxWidth: 380 }}>Sabermetrics is the science of measuring what actually wins games — looking past surface stats like win rate to each player's true value. We bring it to padel: since doubles blends your skill with your partner's, we use your match history across many different partners to isolate what you alone contribute.</p>
    </div>
    <div style={{ ...eyebrow, color: T.subtle, margin: "18px 0 12px" }}>How it works</div>
    {[["01", "We read the match history", "Every Trekkr match records who played with whom, against whom, and the score."], ["02", "We isolate each player", "Because members play with many different partners, the model separates each one's contribution — something win rate can never do."], ["03", "We find the right pairings", "Then it ranks partners by how strong they are and how well their styles click."]].map(([n, t, d]) => <div key={n} style={{ display: "flex", gap: 15, padding: "13px 0", borderTop: `1px solid ${T.borderSoft}` }}><div style={{ ...num, fontSize: 26, ...gradText, lineHeight: 1, flexShrink: 0, width: 34 }}>{n}</div><div><div style={{ ...body, fontWeight: 700, fontSize: 15.5, color: T.text }}>{t}</div><div style={{ ...body, fontSize: 13.5, color: T.muted, lineHeight: 1.55, marginTop: 3 }}>{d}</div></div></div>)}
    <div style={{ ...eyebrow, color: T.subtle, margin: "26px 0 12px" }}>What you get</div>
    <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>{feats.map((f) => <div key={f.t} style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 12, padding: 16, display: "flex", gap: 13 }}><div style={{ width: 38, height: 38, borderRadius: 10, background: T.warm, border: `1px solid ${T.warmBorder}`, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><Icon name={f.ic} /></div><div><div style={{ ...body, fontWeight: 700, fontSize: 15, color: T.text }}>{f.t}</div><div style={{ ...body, fontSize: 13, color: T.muted, lineHeight: 1.5, marginTop: 3 }}>{f.d}</div></div></div>)}</div>
    <div style={{ marginTop: 22, background: T.text, borderRadius: 14, padding: "18px" }}>
    ...
    <button onClick={() => go("venue")} style={{ ...body, width: "100%", padding: "15px 18px", borderRadius: 14, border: "none", cursor: "pointer", background: T.orange, color: "#fff", display: "flex", alignItems: "center", gap: 13, textAlign: "left", boxShadow: `0 10px 30px ${T.orange}40` }}>
      ...
      <span style={{ flex: 1 }}><span style={{ fontWeight: 700, fontSize: 15.5, display: "block" }}>Explore by venue</span><span style={{ fontSize: 12, opacity: .85 }}>Whole-roster analysis for a club or community</span></span>
      <span style={{ fontSize: 18 }}>→</span>
    </button>
    <button onClick={() => go("players")} style={{ ...body, width: "100%", marginTop: 11, padding: "15px 18px", borderRadius: 14, cursor: "pointer", background: T.surf, color: T.text, border: `1.5px solid ${T.orange}`, display: "flex", alignItems: "center", gap: 13, textAlign: "left" }}>
      ...
      <span style={{ flex: 1 }}><span style={{ fontWeight: 700, fontSize: 15.5, display: "block" }}>Explore by players</span><span style={{ fontSize: 12, color: T.muted }}>Search any player and see their analysis</span></span>
...
function PlayerDetail({ hero, scope, count, pro, onOpen, onBack, backLabel }) {
  const animKey = hero.name + pro;
  return <div style={{ animation: "fadeIn .2s ease" }}>
    <button onClick={onBack} style={{ ...body, fontSize: 12.5, fontWeight: 600, color: T.orange, background: "transparent", border: "none", cursor: "pointer", padding: "0 0 12px", display: "flex", alignItems: "center", gap: 5 }}>← {backLabel}</button>
...
function PlayerRowMini({ p, pro, onPick, subtitle, rankNo }) {
  return <button onClick={() => onPick(p)} style={{ ...body, display: "flex", width: "100%", alignItems: "center", gap: 11, padding: "12px 14px", background: "transparent", border: "none", borderTop: rankNo === 1 ? "none" : `1px solid ${T.borderSoft}`, cursor: "pointer", textAlign: "left" }}>
    ...
    <div style={{ flex: 1, minWidth: 0 }}>
      ...
      <div style={{ ...body, fontSize: 10.5, color: T.subtle, marginTop: 2 }}>{subtitle}</div>
    </div>
    {pro ? <span style={{ ...num, fontSize: 19, color: T.text }}>{fmt(p.impact)}</span> : <Locked w={44} />}
    <span style={{ ...body, fontSize: 12, color: T.orange }}>→</span>
  </button>;
}

/* ---------------- Venue mode ---------------- */
function RosterView({ venue, pro, onPick, onOpen }) {
  ...
  return <div style={{ animation: "fadeIn .2s ease" }}>
    <div style={{ marginBottom: 14 }}>
      <h1 style={{ ...head, fontSize: 30, margin: 0, color: T.text }}>{venue.name}</h1>
      <div style={{ ...body, fontSize: 12, color: T.muted, marginTop: 5 }}>{[venue.region, venue.location].filter(Boolean).join(" · ") || "Community"} · {venue.count} players · {venue.rows} matches</div>
    </div>
    <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.surf2, border: `1px solid ${T.border}`, borderRadius: 999, padding: "9px 14px", marginBottom: 14 }}>
      <Icon name="search" /><input value={query} onChange={(e) => setQuery(e.target.value)} placeholder={`Search a player in ${venue.name}`} style={{ ...body, flex: 1, border: "none", outline: "none", background: "transparent", fontSize: 13.5, color: T.text }} />{query && <button onClick={() => setQuery("")} style={{ ...body, border: "none", background: "transparent", color: T.subtle, cursor: "pointer", fontSize: 16, padding: 0 }}>×</button>}
    </div>
    {matches ? <div>{matches.length === 0 ? <div style={{ ...body, fontSize: 13, color: T.subtle, padding: "8px 4px" }}>No player matches “{query}” here.</div> : <div style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden" }}>{matches.slice(0, 10).map((p, i) => <PlayerRowMini key={p.name} p={p} pro={pro} onPick={onPick} rankNo={i + 1} subtitle={`${p.winRate}% win · ${p.apps} matches`} />)}</div>}</div>
      : <>
        {bestDuo && <div style={{ background: pro ? T.warm : T.surf2, border: `1px solid ${pro ? T.warmBorder : T.border}`, borderRadius: 12, padding: "13px 15px", marginBottom: 14 }}>
          <div style={{ ...eyebrow, fontSize: 9, color: pro ? T.orange : T.subtle }}>Strongest duo in the venue</div>
        ...
        <div style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 2px rgba(13,13,13,.04)" }}>
          {roster.map((p, i) => { const delta = p.winRateRank - p.impactRank; return <button key={p.name} onClick={() => onPick(p)} style={{ ...body, display: "flex", width: "100%", alignItems: "center", gap: 11, padding: "12px 14px", background: "transparent", border: "none", borderTop: i === 0 ? "none" : `1px solid ${T.borderSoft}`, cursor: "pointer", textAlign: "left" }}>
...
/* ---------------- Players mode (global) ---------------- */
function PlayersView({ globalPlayers, pro, onPick }) {
  const [query, setQuery] = useState("");
  const matches = query ? globalPlayers.filter((p) => p.name.toLowerCase().includes(query.toLowerCase())) : null;
  const top = globalPlayers.slice(0, 15);
  return <div style={{ animation: "fadeIn .2s ease" }}>
    <div style={{ marginBottom: 14 }}><h1 style={{ ...head, fontSize: 30, margin: 0, color: T.text }}>All players</h1><div style={{ ...body, fontSize: 12, color: T.muted, marginTop: 5 }}>{globalPlayers.length} players across every venue · search any name</div></div>
    <div style={{ display: "flex", alignItems: "center", gap: 8, background: T.surf2, border: `1px solid ${T.border}`, borderRadius: 999, padding: "9px 14px", marginBottom: 14 }}>
    ...
    {matches ? (matches.length === 0 ? <div style={{ ...body, fontSize: 13, color: T.subtle, padding: "8px 4px" }}>No player matches “{query}”.</div> : <div style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden" }}>{matches.slice(0, 12).map((p, i) => <PlayerRowMini key={p.name} p={p} pro={pro} onPick={onPick} rankNo={i + 1} subtitle={`${p.venue} · ${p.winRate}% win`} />)}</div>)
      : <><div style={{ ...eyebrow, color: T.subtle, margin: "4px 4px 8px" }}>Top players overall</div><div style={{ background: T.surf, border: `1px solid ${T.border}`, borderRadius: 14, overflow: "hidden", boxShadow: "0 1px 2px rgba(13,13,13,.04)" }}>{top.map((p, i) => <PlayerRowMini key={p.name} p={p} pro={pro} onPick={onPick} rankNo={i + 1} subtitle={`${p.venue} · ${p.winRate}% win`} />)}</div>
        {!pro && <div style={{ ...body, fontSize: 12.5, color: "#8A5A00", background: T.warm, border: `1px solid ${T.warmBorder}`, borderRadius: 10, padding: "11px 13px", marginTop: 12, lineHeight: 1.5 }}>Free shows the roster. <b>Unlock Pro</b> to reveal each player's true impact and partner fit.</div>}</>}
  </div>;
}

function Analysis({ onOpen, mode, setMode, venueName, setVenueName, player, setPlayer }) {
  ...
  const detailScope = player ? (player.venue || venue.name) : "";
  const detailCount = player ? (player.venueCount || venue.count) : 0;
  ...
  return <div style={{ padding: "16px 18px 40px", animation: "fadeIn .25s ease" }}>
    ...
    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 14, gap: 10 }}>
      <div style={{ display: "flex", background: T.surf2, borderRadius: 999, padding: 3, border: `1px solid ${T.border}` }}>
        {[["venue", "By venue"], ["players", "By players"]].map(([m, label]) => { const on = mode === m; return <button key={m} onClick={() => { setMode(m); setPlayer(null); }} style={{ ...body, fontSize: 11.5, padding: "6px 12px", borderRadius: 999, border: "none", cursor: "pointer", color: on ? "#fff" : T.muted, background: on ? T.text : "transparent", fontWeight: 700, transition: "all .18s" }}>{label}</button>; })}
      </div>
    ...
    {mode === "venue" && !player && <div style={{ position: "relative", marginBottom: 14 }}>
    ...
    <button onClick={() => onOpen("__glossary__")} style={{ ...body, fontSize: 12, fontWeight: 600, color: T.orange, background: T.warm, border: `1px solid ${T.warmBorder}`, borderRadius: 10, padding: "9px 12px", cursor: "pointer", width: "100%", textAlign: "left", marginBottom: 16, display: "flex", alignItems: "center", justifyContent: "space-between" }}><span>New here? Read “How to read this” first</span><span>→</span></button>

    {player ? <PlayerDetail hero={player} scope={detailScope} count={detailCount} pro={pro} onOpen={onOpen} onBack={() => setPlayer(null)} backLabel={mode === "venue" ? `${venue.name} roster` : "All players"} />
      : mode === "venue" ? <RosterView venue={venue} pro={pro} onPick={setPlayer} onOpen={onOpen} />
        : <PlayersView globalPlayers={globalPlayers} pro={pro} onPick={setPlayer} />}

    <div style={{ ...body, fontSize: 11, color: T.subtle, textAlign: "center", marginTop: 18, lineHeight: 1.6 }}>{pro ? "Prototype · real data from trekkr.online" : "Free shows the insight exists — Pro unlocks it"}</div>
  </div>;
}
...
export default function App() {
  ...
  return <div style={{ minHeight: "100vh", background: T.bg, color: T.text, ...body, display: "flex", justifyContent: "center" }}>
    <style>{FONTS}</style>
    <div style={{ width: "100%", maxWidth: 440, borderLeft: `1px solid ${T.borderSoft}`, borderRight: `1px solid ${T.borderSoft}`, minHeight: "100vh" }}>
      <Header page={page} onBack={onBack} backLabel={player ? "← Back" : "← Overview"} />
      {page === "intro" ? <Intro go={go} onOpen={setDef} /> : <Analysis onOpen={setDef} mode={mode} setMode={setMode} venueName={venueName} setVenueName={setVenueName} player={player} setPlayer={setPlayer} />}
    </div>
